...
var count = function count() {
  var songList = JSON.parse(window.localStorage.getItem('songs'));
  ...
  return {
    ...
    addCount: function addCount() {
      ...
      if (this.getCount < listLength) {
        var getLastCount = window.localStorage.getItem('count');
        ...
        if (getLastCount > 0) {
          ...
          var _getLastCount = window.localStorage.getItem('count');

          this.getCount = _getLastCount;
        }
        ...
        var runnerup = document.querySelector(".runnerup-name");
        var winnerPoint = document.querySelector(".winner-point");
        var runnerupPoint = document.querySelector(".runnerup-point");
    ...
    getEmojis: function getEmojis() {
      var currSong = this.start();
      return currSong["emojis"];
    },
    getSong: function getSong() {
      var currSong = this.start();
      return "./src/songs/".concat(currSong['song']);
    },
    getFullTitle: function getFullTitle() {
      var currSong = this.start();
      var title = currSong["title"];
    ...
    hints: function hints() {
      ...
      var hintCount = parseInt(existing["hintCount"]);

      if (hintCount < 3) {
        hintCount++;
        existing["hints"]["Hint".concat(hintCount)] = this.randomNum();
    ...
    randomNum: function randomNum() {
      var numHints = [];
      this.getTitle().forEach(function (arr) {
        numHints.push(Math.floor(Math.random() * arr.length));
      });
      return numHints;
    },
    removeHints: function removeHints() {
      ...
      var letter = document.querySelectorAll(".song_letter li");
      letter.forEach(function (el) {
        return el.getAttribute("style") ? el.style.visibility = "hidden" : '';
      });
    },
    ...
    showHint: function showHint() {
      ...
      if (existing) {
        (function () {
          existing = JSON.parse(existing);
          var title_container = document.querySelectorAll(".song_wrap");
          var getRand = existing["hints"];

          var _loop = function _loop(i) {
            var j = 0;
            title_container.forEach(function (list) {
              var letter = list.querySelectorAll(".song_letter li");
              letter[getRand["Hint".concat(i)][j]].style.visibility = "visible";
              j++;
            });
          };

          for (var i = 1; i <= existing["hintCount"]; i++) {
            _loop(i);
          }
        })();
      }
...
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data("count", count);
...
var players = function players(config) {
  return {
    ...
    savePlayer: function savePlayer(name) {
      var players = JSON.parse(window.localStorage.getItem("players"));
      players.push(name);
    ...
    saveScore: function saveScore(key, score) {
...
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data("players", players);
...
var timerClock = function timerClock() {
  return {
    timeLeft: 10,
...
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data("timerClock", timerClock);
...
window.onload = function () {
  ...
  function poof() {
    if (!frame) {
      ...
      var resultbox = document.querySelector('.final-results');
      resultbox.appendChild(container); // Add confetti
...
/* harmony import */ var _winner__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./winner */ "./src/winner.js");
/* harmony import */ var _winner__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_winner__WEBPACK_IMPORTED_MODULE_3__);
...
var BasePage = /*#__PURE__*/function () {
  ...
  _createClass(BasePage, [{
    ...
    value: function onReady() {
      _game__WEBPACK_IMPORTED_MODULE_4__["default"].init();
    }
  }]);
